...
\end{lstlisting}
        \subsection{Evaluation results}
            ...
            Optimal caching has the best compression rate (Fig.~\ref{graph:compmem}), but the worst average number of instructions required for processing one packet (Fig.~\ref{graph:compinst}).
            This can be explained by the CPU calling process take up a lot of instructions.

            Recursive end-point cutting has less compression rate then optimal caching (Fig.~\ref{graph:compmem}) 
            and better average number of instruction required for processing one packet (Fig.~\ref{graph:compinst}).
            \begin{figure*}[!t]
                \centering
